...
process for free:
	use pointer passed into method, 
	subtract sizeof(meta) from address of pointer, 
	cast struct at that address as meta data, 
	set free to 1 so that next time malloc is called metadata is erased and memory is allocated to the end of block for reassignment 
